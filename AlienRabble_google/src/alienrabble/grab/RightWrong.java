...
public class RightWrong extends Node {
    ...
    public RightWrong(DisplaySystem display, String name){
        ...
        ts.setTexture(TextureManager.loadTexture(getClass().getClassLoader()
                .getResource("alienrabble/data/texture/rightwrong.png"), Texture.MinificationFilter.BilinearNearestMipMap,
    ...
    private void changeRightWrongQuad(){
		...
		fbuffer.put(getUForPixel(u)).put(getVForPixel(v+width));
		fbuffer.put(getUForPixel(u+height)).put(getVForPixel(v+width));
		fbuffer.put(getUForPixel(u+height)).put(getVForPixel(v));
		TexCoords texCoords = new TexCoords();
		texCoords.coords = fbuffer;
        rightwrongQuad.setTextureCoords(texCoords, 0);	
    }
